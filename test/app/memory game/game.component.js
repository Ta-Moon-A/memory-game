"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var card_list_component_1 = require("./card.list.component");
var setting_component_1 = require("./setting.component");
var GameComponent = (function () {
    function GameComponent() {
        this.gameMode = false;
    }
    GameComponent.prototype.onSettingClicked = function () {
        this.gameMode = false;
    };
    GameComponent.prototype.onStartGameClicked = function (gameProps) {
        this.gameMode = true;
        this.gameProps = gameProps;
    };
    GameComponent = __decorate([
        core_1.Component({
            selector: "memory-game",
            template: "<card-list *ngIf=\"gameMode\" (settingClicked)=\"onSettingClicked()\" [gameProperties]=\"gameProps\"></card-list>\n               <setting *ngIf=\"!gameMode\" (gameStartClicked)=\"onStartGameClicked($event)\"></setting>",
            directives: [card_list_component_1.CardListComponent, setting_component_1.SettingComponent]
        }), 
        __metadata('design:paramtypes', [])
    ], GameComponent);
    return GameComponent;
}());
exports.GameComponent = GameComponent;
//# sourceMappingURL=game.component.js.map